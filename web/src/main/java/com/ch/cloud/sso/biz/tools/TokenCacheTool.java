...
/**
 * Token Redis工具类 封装RedisTemplate，提供Token与用户信息映射的管理功能 支持多平台Token管理和授权码机制
 *
 * @author zhimin
 * @since 2025/1/1
 */
...
public class TokenCacheTool {
    ...
    public void saveAccessToken(String token, TokenDTO tokenDTO, long expireTime) {
        try {
            ...
            log.debug("保存访问Token成功: {}, 用户: {}, 平台: {}, 过期时间: {}秒", token, tokenDTO.getUsername(),
                    tokenDTO.getReferer(), expireTime);
        } catch (Exception e) {
    ...
    public void saveRefreshToken(String refreshToken, RefreshTokenDTO refreshTokenDTO, long expireTime) {
        try {
            ...
            stringRedisTemplate.opsForValue()
                    .set(refreshTokenUserKey, refreshTokenDTO.getUsername(), expireTime, TimeUnit.SECONDS);

            // 保存用户与刷新Token的映射关系
            stringRedisTemplate.opsForValue().set(userRefreshKey, refreshToken, expireTime, TimeUnit.SECONDS);

            log.debug("保存刷新Token成功: {}, 用户: {}, 过期时间: {}秒", refreshToken, refreshTokenDTO.getUsername(),
                    expireTime);
        } catch (Exception e) {
    ...
    public String consumeAuthCode(String authCode) {
        String authCodeKey = AUTH_CODE_PREFIX + authCode;
        Assert.isTrue(stringRedisTemplate.hasKey(authCodeKey), PubError.INVALID, "授权码无效或已过期");
        String username = stringRedisTemplate.opsForValue().get(authCodeKey);
...
//        stringRedisTemplate.opsForSet().remove(userAuthCodesKey, authCode);
     ...
     * @return 是否同步成功
     */
    public boolean syncUserTokenToPlatform(String username, String sourcePlatform, String targetPlatform,
            long expireTime) {
        try {
            // 获取源平台的Token信息
    ...
    public boolean renewAccessToken(String token, long expireTime) {
        try {
            ...
            if (tokenDTO != null) {
                ...
                log.debug("续期访问Token成功: {}, 用户: {}, 新过期时间: {}秒", token, tokenDTO.getUsername(),
                        expireTime);
                return true;
            }
            return false;
     ...
     * @return 是否续期成功
     */
    public boolean refreshToken(String refreshToken, long expireTime) {
        try {
            ...
            if (refreshTokenDTO != null) {
                ...
                log.debug("续期刷新Token成功: {}, 用户: {}, 新过期时间: {}秒", refreshToken,
                        refreshTokenDTO.getUsername(), expireTime);
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    /**
     * 批量删除过期的Token 注意：这个方法需要谨慎使用，可能会影响性能
     */
