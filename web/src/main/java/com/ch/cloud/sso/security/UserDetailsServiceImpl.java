...
import com.ch.cloud.sso.biz.pojo.BtnVo;
...
//        vo.setHidden(permission.isHidden());
        ...
        String pid = CommonUtils.isEquals(Num.S0, permission.getParentId()) ? permission.getId().toString() : StrUtil.linkStr(",", permission.getParentId(), permission.getId().toString());
...
//        log.info("get user roles: {}", JSON.toJSONString(res2));
        if (res2.isEmpty()) {
            log.warn("用户[{}]角色不存在！", user.getUsername());
        ...
        if (!roleIds.contains(user.getRoleId())) {
            log.warn("用户{}角色[{}]不存在！自动切换已有角色[{}]",user.getUsername(), user.getRoleId(),roleIds.get(0));
