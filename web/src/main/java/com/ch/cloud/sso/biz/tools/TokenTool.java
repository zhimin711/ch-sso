...
public class TokenTool {
    ...
    public String getUsernameFromToken(String token) {
        // 获取token缓存map
        RMapCache<String, TokenDTO> tokenMap = redissonClient.getMapCache(TOKEN_CACHE, JsonJacksonCodec.INSTANCE);
    ...
    public boolean isTokenExpired(String token) {
        try {
            // 获取token缓存map
            RMapCache<String, TokenDTO> tokenMap = redissonClient.getMapCache(TOKEN_CACHE, JsonJacksonCodec.INSTANCE);
            TokenDTO cache = tokenMap.get(token);
            if (cache == null) {
                // 缓存不存在，视为过期
                return true;
            }
    ...
    public void refreshToken(TokenVo tokenVo) {
        ...
        try {
            ...
            // 获取刷新token缓存
            RMapCache<String, RefreshTokenDTO> refreshTokenMap = redissonClient.getMapCache(REFRESH_TOKEN_CACHE,
                    JsonJacksonCodec.INSTANCE);
            RefreshTokenDTO cache2 = refreshTokenMap.get(tokenVo.getRefreshToken());
    ...
    public UserInfo getUserInfoFromRefreshToken(String token) {
        // 获取refreshToken缓存map
        RMapCache<String, RefreshTokenDTO> tokenMap = redissonClient.getMapCache(REFRESH_TOKEN_CACHE,
                JsonJacksonCodec.INSTANCE);
        RefreshTokenDTO cache = tokenMap.get(token);
    ...
    private void fillRequestInfo(TokenDTO tokenDTO) {
        // 获取当前请求上下文
        ...
        tokenDTO.setHost(request.getHeader("Host"));
        tokenDTO.setUserAgent(request.getHeader("User-Agent"));
        String ip = getIP(request);
        tokenDTO.setClientIp(ip);
        tokenDTO.setReferer(request.getHeader("Referer"));
    }
    ...
    public TokenVo authToken(String authCode) {
        ...
        RMapCache<String, RefreshTokenDTO> refreshTokenMap = redissonClient.getMapCache(REFRESH_TOKEN_CACHE,
                JsonJacksonCodec.INSTANCE);
        // 获取refreshToken缓存对象
        RefreshTokenDTO refreshTokenDTO = refreshTokenMap.get(refreshToken);
        // 生成新令牌对应的用户信息
        UserInfo userInfo = getUserInfoFromRefreshToken(refreshTokenDTO);
        // 克隆用户信息到TokenCache
        TokenDTO authTokenDTO = BeanUtilsV2.clone(userInfo, TokenDTO.class);
        fillRequestInfo(authTokenDTO);
        ...
        authTokenDTO.setExpireAt(accessExpired.getTime());
        // 存入token缓存
        tokenMap.put(accessToken, authTokenDTO, jwtProperties.getTokenExpired().getSeconds(), TimeUnit.SECONDS);
        // 构造返回对象
        TokenVo tokenVo = new TokenVo();
        tokenVo.setToken(accessToken);
        tokenVo.setRefreshToken(refreshToken);
        tokenVo.setExpireAt(authTokenDTO.getExpireAt());
        // 维护refreshToken与accessToken的多值映射
        RSetMultimapCache<String, String> tokensCache = redissonClient.getSetMultimapCache(TOKEN_CACHE);
    ...
    public UserInfo getUserInfoFromToken(String token) {
        ...
        TokenDTO tokenDTO = tokenMap.get(token);
