...
public class ImageVerifyUtil {
     ...
     * @return 裁剪坐标
     */
    public static SlideCaptcha generateCutoutCoordinates(BufferedImage verificationImage, BufferedImage templateImage) {
...
//        int VERIFICATION_IMAGE_HEIGHT = verificationImage.getHeight();
     ...
     * @return
     */
    public static SlideCaptcha pictureTemplateCutout(File originImageFile, String originImageFileType, File templateImageFile, String templateImageFileType, int x, int y) {
        SlideCaptcha slideCaptcha = null;
        ...
        try {
            ...
            slideCaptcha.setOrigin(originImageString);
            ...
            slideCaptcha.setShade(shadeImageString);
            ...
            slideCaptcha.setPuzzle(cutoutImageString);


        } catch (IOException e) {
            log.error(e.getMessage(), e);
            ExceptionUtils._throw(PubError.DEFAULT);
        } finally {
...
//                    shadeImage.setRGB(x + i, y + j, originRGB);
     ...
     * @return
     */
    private static BufferedImage getInterestArea(int x, int y, int templateImageWidth, int templateImageHeight, File originImage, String originImageType) {
     ...
     * 切块图描边
     *
     * @param slideCaptcha 图片容器
     * @param borderImage         描边图
     * @param borderImageFileType 描边图类型
     * @return 图片容器
     */
    public static SlideCaptcha cutoutImageEdge(SlideCaptcha slideCaptcha, BufferedImage borderImage, String borderImageFileType) {

        ByteArrayInputStream byteArrayInputStream = null;
        ByteArrayOutputStream byteArrayOutputStream = null;

        try {
            String cutoutImageString = slideCaptcha.getPuzzle();
            //  图片解密成二进制字符创
